...
public class VanillaGuiHandler {
    ...
    public static void disableVanillaAarmor(RenderGuiOverlayEvent.Pre event) {
        ...
        if (!event.isCanceled()
                && event.getOverlay().id().equals(vanillAarmor_level)
                && !mc.options.hideGui
    ...
    public static void disableVanillaMount(RenderGuiOverlayEvent.Pre event) {
        final ForgeGui gui = ((ForgeGui) mc.gui);
        if (!event.isCanceled()
                && event.getOverlay().id().equals(vanillaMount_health)
                && !mc.options.hideGui
                && gui.shouldDrawSurvivalElements()
                && mc.cameraEntity instanceof Player) {
            event.setCanceled(true);
    ...
    public static void disableVanillaFood(RenderGuiOverlayEvent.Pre event) {
        final ForgeGui gui = ((ForgeGui) mc.gui);
        if (!event.isCanceled()
                && event.getOverlay().id().equals(vanillaFoodBar)
                && !mc.options.hideGui
                && gui.shouldDrawSurvivalElements()
                && mc.cameraEntity instanceof Player) {
            event.setCanceled(true);
    ...
    public static void disableVanillaAir(RenderGuiOverlayEvent.Pre event) {
        final ForgeGui gui = ((ForgeGui) mc.gui);
        if (!event.isCanceled()
                && event.getOverlay().id().equals(vanillaAir)
                && !mc.options.hideGui
                && gui.shouldDrawSurvivalElements()
                && mc.cameraEntity instanceof Player) {
            event.setCanceled(true);
